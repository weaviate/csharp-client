    ...
    where T : class, new()
{
    ...
    /// </code>
    /// </example>
    public CollectionMapperQueryClient<T> WithVectors(params Expression<Func<T, object>>[] vectors)
    {
    ...
    public async Task<IEnumerable<T>> ExecuteAsync(CancellationToken cancellationToken = default)
    {
        ...
        return result.Objects.Select(CollectionMapperObjectMapper.FromWeaviateObject);
    }
