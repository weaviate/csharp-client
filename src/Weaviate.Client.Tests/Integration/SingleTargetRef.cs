...
public partial class BasicTests
{
    ...
    public async Task SingleTargetReference()
    {
        ...
        var uuid_A1 = await cA.Data.Insert(DataFactory(new TestData() { Name = "A1" }));
        var uuid_A2 = await cA.Data.Insert(DataFactory(new TestData() { Name = "A2" }));
        ...
        var uuid_B = await cB.Data.Insert(
            DataFactory(new TestData { Name = "B" }),
        ...
        var uuid_C = await cC.Data.Insert(DataFactory(new TestData { Name = "find me" }),
        new Dictionary<string, Guid> {
                { "b", uuid_B }
        });

        // Act
        var aObjs = (await cA.Query.BM25(query: "A1", ["name"])).ToList();
        ...
        // Assert
        Assert.Equal(cA.Name, aObjs[0].CollectionName);
        Assert.NotNull(aObjs[0].Data?.Name);
        Assert.Equal("A1", aObjs[0].Data?.Name);

        Assert.Equal(cA.Name, bObjs[0].References["a"][0].CollectionName);
        Assert.Equal("A1", bObjs[0].References["a"][0].Properties["name"]);
        Assert.Equal(uuid_A1, bObjs[0].References["a"][0].ID);
        Assert.Equal(cA.Name, bObjs[0].References["a"][1].CollectionName);
        Assert.Equal("A2", bObjs[0].References["a"][1].Properties["name"]);
        Assert.Equal(uuid_A2, bObjs[0].References["a"][1].ID);


        Assert.Equal(cC.Name, cObjs[0].CollectionName);
        Assert.Equal("find me", cObjs[0].Properties["name"]);
        Assert.Equal(uuid_C, cObjs[0].ID);
        Assert.Equal(cB.Name, cObjs[0].References["b"][0].CollectionName);
        Assert.Equal("B", cObjs[0].References["b"][0].Properties["name"]);
        Assert.NotNull(cObjs[0].References["b"][0].Metadata.LastUpdateTime);
        Assert.Equal(cA.Name, cObjs[0].References["b"][0].References["a"][0].CollectionName);
        Assert.Equal("A1", cObjs[0].References["b"][0].References["a"][0].Properties["name"]);
        Assert.Equal(cA.Name, cObjs[0].References["b"][0].References["a"][1].CollectionName);
        Assert.Equal("A2", cObjs[0].References["b"][0].References["a"][1].Properties["name"]);
    }
    ...
    public async Task SingleTargetReference_MovieReviews()
    {
        ...
        var reviews = await CollectionFactory<dynamic>(
            ...
            properties: [
                ...
                Property.Int("movie_id"),
                Property.Reference("forMovie", targetCollection: "Movie"),
            ],
        ...
        {
            var uuid = await movies.Data.Insert(DataFactory<dynamic>(m));
...
It won’t make the regular rotation of our traditional holiday movies, but I am glad we fit it in this year.",
        ...
        // Act
        var fun = await reviews.Query.NearText("Fun for the whole family", limit: 2).ToListAsync(TestContext.Current.CancellationToken);
        Console.WriteLine(JsonSerializer.Serialize(fun, new JsonSerializerOptions { WriteIndented = true }));
        ...
        Console.WriteLine(JsonSerializer.Serialize(disappointed, new JsonSerializerOptions { WriteIndented = true }));

        // Assert
        Assert.NotNull(fun);
        Assert.Equal(2, fun.Count);
        Assert.Equal(0, fun[0]?.References.Count);
        Assert.Equal(0, fun[1]?.References.Count);


        Assert.NotNull(disappointed);
        Assert.Equal(2, disappointed.Count);
        Assert.Equal(1, disappointed[0]?.References.Count);
        Assert.Equal(1, disappointed[1]?.References.Count);

        Assert.True(disappointed[0].References.ContainsKey("forMovie"));
        Assert.True(disappointed[1].References.ContainsKey("forMovie"));
        Assert.Equal(movieIds[162], disappointed[0].References["forMovie"][0].ID);
        Assert.Equal(movieIds[771], disappointed[1].References["forMovie"][0].ID);
    }
}
