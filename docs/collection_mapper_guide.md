# Weaviate.Client.CollectionMapper - Complete Guide
...
The Weaviate WOMP layer provides a type-safe, attribute-based approach to working with Weaviate collections in C#. It eliminates the need for manual schema definitions and provides LINQ-style querying, automatic object mapping, and schema migration support.
...
The WOMP is built as an extension layer on top of the existing `Weaviate.Client` library. It never modifies the core client and uses extension methods to add WOMP capabilities while maintaining full access to the underlying client API.
...
```bash
dotnet add package Weaviate.Client.CollectionMapper
```

Or add to your `.csproj`:

```xml
<PackageReference Include="Weaviate.Client.CollectionMapper" Version="1.0.0" />
```
...
The WOMP automatically converts C# property names (PascalCase) to Weaviate property names (camelCase):
...
The WOMP provides a safe migration system for evolving schemas over time.
...
```csharp
// Get WOMP query builder
CollectionMapperQueryClient<T> Query<T>() where T : class
```
...
// Vector search
CollectionMapperQueryClient<T> NearText(
    string text,
    Expression<Func<T, object>>? vector = null,
    float? certainty = null,
    float? distance = null)

CollectionMapperQueryClient<T> NearVector(
    float[] vector,
...
// Hybrid search
CollectionMapperQueryClient<T> Hybrid(
    string query,
    float? alpha = null,
    Expression<Func<T, object>>? vector = null)

// Include data
CollectionMapperQueryClient<T> WithVectors(Expression<Func<T, object>> vectorProperty)
CollectionMapperQueryClient<T> WithReferences(Expression<Func<T, object>> referenceProperty)
CollectionMapperQueryClient<T> Select(Expression<Func<T, object>> property)
CollectionMapperQueryClient<T> WithMetadata(MetadataQuery metadata)

// Sorting and pagination
CollectionMapperQueryClient<T> Sort(Expression<Func<T, object>> property, bool descending = false)
CollectionMapperQueryClient<T> Limit(long limit)
CollectionMapperQueryClient<T> Offset(long offset)
...
T CollectionMapperObjectMapper.FromWeaviateObject<T>(WeaviateObject weaviateObject)
List<T> CollectionMapperObjectMapper.FromWeaviateObjects<T>(List<WeaviateObject> weaviateObjects)

// Check if mapping needed
bool CollectionMapperObjectMapper.RequiresMapping<T>()
```
...
// Avoid - using raw filters if WOMP is available
...
// 3. Use WOMP with existing collection
...
- `src/Weaviate.Client.CollectionMapper/Examples.cs` - Comprehensive examples
- `src/Weaviate.Client.CollectionMapper.Tests/` - Unit tests demonstrating usage
