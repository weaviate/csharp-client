# Weaviate.Client.Womp - Complete Guide
...
```bash
dotnet add package Weaviate.Client.Womp
```

Or add to your `.csproj`:

```xml
<PackageReference Include="Weaviate.Client.Womp" Version="1.0.0" />
```
...
WompQueryClient<T> Query<T>() where T : class
```
...
```csharp
// Filtering
WompQueryClient<T> Where(Expression<Func<T, bool>> predicate)

// Vector search
WompQueryClient<T> NearText(
    string text,
    Expression<Func<T, object>>? vector = null,
    float? certainty = null,
    float? distance = null)

WompQueryClient<T> NearVector(
    float[] vector,
...
// Hybrid search
WompQueryClient<T> Hybrid(
    string query,
    float? alpha = null,
    Expression<Func<T, object>>? vector = null)

// Include data
WompQueryClient<T> WithVectors(Expression<Func<T, object>> vectorProperty)
WompQueryClient<T> WithReferences(Expression<Func<T, object>> referenceProperty)
WompQueryClient<T> Select(Expression<Func<T, object>> property)
WompQueryClient<T> WithMetadata(MetadataQuery metadata)

// Sorting and pagination
WompQueryClient<T> Sort(Expression<Func<T, object>> property, bool descending = false)
WompQueryClient<T> Limit(long limit)
WompQueryClient<T> Offset(long offset)
...
T WompObjectMapper.FromWeaviateObject<T>(WeaviateObject weaviateObject)
List<T> WompObjectMapper.FromWeaviateObjects<T>(List<WeaviateObject> weaviateObjects)

// Check if mapping needed
bool WompObjectMapper.RequiresMapping<T>()
```
...
- `src/Weaviate.Client.Womp/Examples.cs` - Comprehensive examples
- `src/Weaviate.Client.Womp.Tests/` - Unit tests demonstrating usage
