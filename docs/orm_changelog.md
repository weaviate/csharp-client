# Weaviate.Client.Orm - Development Changelog

## Purpose

This document tracks all implementation decisions, changes, and progress during the development of the Weaviate C# ORM layer. It serves as a historical record and context for future development.
...
### Summary
Successfully implemented the complete Query Builder system with LINQ-style fluent API. The ORM now supports type-safe querying with expression trees, vector search, and full integration with the existing TypedQueryClient.
...
**Purpose:** Extension method to get ORM query client

**Method:**
```csharp
public static OrmQueryClient<T> Query<T>(this CollectionClient collection)
    where T : class, new()
```
...
- **Location:** OrmQueryClient.cs:184
...
- **Location:** OrmQueryClient.cs:312
...
#### Decision 1: Separate ORM Project
**Decision:** Create `Weaviate.Client.Orm` as a separate project/package
**Rationale:**
- Keeps core client lean and unchanged
- Users can opt-in to ORM features
- Easier to evolve ORM independently
...
- ORM can evolve independently
- Users can mix ORM and raw client
- Easier to maintain and test
**Implementation:**
- Extension methods: `collection.Query<T>()`
- Wrappers: `OrmQueryClient<T>` wraps `TypedQueryClient<T>`
...
4. **Wrapper/Decorator Pattern** - ORM wraps existing client
...
**Next Steps:**
1. Create `Weaviate.Client.Orm` project structure
...
**Action:** Implemented core ORM infrastructure for schema building
**Status:** ✅ Schema building functionality complete and ready for testing

**Files Created:**
1. **Project Structure**
   - `src/Weaviate.Client.Orm/Weaviate.Client.Orm.csproj` - Project file with Humanizer.Core dependency
